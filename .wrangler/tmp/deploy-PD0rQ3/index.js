...
var ALIASES = {
  ...
  "otp": "auth",
  "2fa": "auth",
  "session expir\xE9e": "auth",
  "acc\xE8s refus\xE9": "auth",
  "identifiant": "auth",
  "r\xE9initialiser mot de passe": "auth",
  "ticket": "support",
  "tickets": "support",
  "complaint": "support",
...
__name(toStr, "toStr");
var DASH_BASE = "https://dash.tiktak.space";
...
- \xC9tapes num\xE9rot\xE9es 1..3, **Gras** pour menus/boutons. CHAQUE \xE9tape sur sa propre ligne (\\n entre chaque).
- Format: "Texte intro :\\n1. \xC9tape un\\n2. \xC9tape deux\\n3. \xC9tape trois\\n\\nConclusion"
...
function processLlmDiagnosis(opts) {
  ...
  const isHowTo = /comment|how\s+to|tuto|explication|c['’]est\s+quoi/i.test(message) || llmTicketType === "question";
  ...
  if (BANNED_TERMS_RE.test(finalAnswer)) {
    finalAnswer = finalAnswer.split("\n").filter((l) => !BANNED_TERMS_RE.test(l)).join("\n").trim() || "Je suis l\xE0 pour t'aider \u{1F44D} Dis-moi ce que tu vois exactement (message d'erreur / page) et on avance.";
  }
  ...
  const route_link = routeFor(finalModule, void 0);
