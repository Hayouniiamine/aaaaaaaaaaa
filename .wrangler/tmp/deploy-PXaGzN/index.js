...
PERSONNALIT\xC9: professionnel, chaleureux, efficace. Tu tutoies.
...
=== M\xC9THODE (DIAGNOSTIC AVANT PRESCRIPTION) ===
R\xE8gle: diagnostiquer avant de prescrire.
Exception: si la demande est clairement "comment faire X ?" \u2192 donne directement 2-3 \xE9tapes.

1) Probl\xE8me vague ("\xE7a marche pas", "bloqu\xE9") \u2192 verdict="unclear" + next_question (1 question pr\xE9cise: message exact / page / depuis quand).
2) Ne r\xE9p\xE8te jamais les m\xEAmes \xE9tapes. Si "tout est v\xE9rifi\xE9" / "persiste" \u2192 propose une ALTERNATIVE ou question plus cibl\xE9e.
3) 2-3 \xE9tapes max par r\xE9ponse. Attends le retour.
4) Utilise l'historique. Ne redemande pas une info d\xE9j\xE0 donn\xE9e.

=== STYLE (answer) ===
- Reconnaissance br\xE8ve ("Je vois que\u2026", "Bien re\xE7u !")
...
- Terminer par suivi ("Dis-moi si \xE7a fonctionne !" / "Tiens-moi au courant")
...
__name(lastAssistantText, "lastAssistantText");
function normText(s) {
...
function buildHistoryStateSummary(history, currentModule) {
  ...
  const parts = [];
  parts.push(`\xC9TAT CONVERSATION: ${assistantMsgs.length} r\xE9ponses d\xE9j\xE0 donn\xE9es.`);
  ...
  if (lastAnswer.length > 30) {
    parts.push(`Derni\xE8re r\xE9ponse (r\xE9sum\xE9): "${lastAnswer.slice(0, 150)}\u2026"`);
  }
  parts.push("NE R\xC9P\xC8TE PAS les m\xEAmes \xE9tapes. Propose une alternative ou pose une question cibl\xE9e.");
  return "\n--- \xC9TAT M\xC9MOIRE ---\n" + parts.join("\n");
}
  ...
  if (mode === "solve" || mode === "clarify") {
    ...
    if (prev && prev.length > 30) {
      ...
      if (sim >= 0.65) {
        ...
        if (mod === "settings") {
          nextQuestion = "Tu peux m'envoyer le nom exact du domaine + le message affich\xE9 (ex: SSL 525/526, site introuvable, page blanche) et depuis quand \xE7a arrive ?";
          ...
          nextQuestion = "Tu peux me donner le num\xE9ro de commande + ce que tu vois exactement (erreur, statut, page bloqu\xE9e) ?";
          ...
          nextQuestion = "Quel transporteur + quel num\xE9ro de commande / colis ? Qu'est-ce que tu vois exactement (erreur, statut, tracking) ?";
        } else {
          nextQuestion = "Donne-moi une pr\xE9cision (message d'erreur / capture / depuis quand) pour \xE9viter de te refaire les m\xEAmes \xE9tapes.";
        }
        finalAnswer = "Bien re\xE7u \u2014 si tout est d\xE9j\xE0 v\xE9rifi\xE9, je veux \xE9viter de te r\xE9p\xE9ter les m\xEAmes \xE9tapes. J'ai juste besoin d'une petite pr\xE9cision pour diagnostiquer correctement.";
      }
    }
  }
